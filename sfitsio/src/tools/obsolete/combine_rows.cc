 ...
 *
 * ./combine_images -p "combine=... outtype=..." in.fits -o out.fits"
 *
 */
int main( int argc, char *argv[] )
{
	...
	/* combine */
	tmppri.combine_rows(&outfits.image("Primary"), combine_options.cstr());
 ...
 usage_quit:
    /* display usage */
    if ( argc == 1 ) {
       sio.printf("combine_rows: combine rows in a Primary image.\n");
       sio.printf("[USAGE]\n");
