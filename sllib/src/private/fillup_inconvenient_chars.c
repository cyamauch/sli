...
/* Time-stamp: <2008-09-13 03:45:52 cyamauch> */
 ...
 * と設定する．
 */
#define FUNC_MSG "fillup_inconvenient_chars(): "
static int fillup_inconvenient_chars( const char *target_string, 
				      ssize_t len_targetstring,
				      const char *inconvenient, 
				      const char *quotations , 
				      int escape, int substitute,
				      char **return_filled )
{
    ssize_t i;
    int qcount;
    ...
    *return_filled=filled;
    ...
    /*「バックスラッシュ(escape)+何か」の場合は __(substitute) に置換 */
    ...
    /* 括弧(例えば "") の中身については，inconvenient にあるもの
       (例えば <，> ，スペース，タブ) は substitute (例えば _) に置き換え */
    qcount=0;
	...
	for ( i=0 ; i<len_targetstring ; i++ ) {
		...
		if ( qcount % 2 != 0 ) {
			...
			filled[i] = substitute;	/* 例えば _ に置き換え */
		    }
		}
	    }
	}
