...
#ifndef _SLI__MDARRAY_UCHAR_H
#define _SLI__MDARRAY_UCHAR_H 1

/**
 * @file   mdarray_uchar.h
 * @brief  unsigned char型の多次元配列を扱う mdarray_uchar クラスのヘッダファイル
 */
...
/*
 * sli::mdarray_uchar is an inherited class of sli::mdarray, and is a 
...
/**
 * @class  sli::mdarray_uchar
 * @brief  unsigned char型の多次元配列を扱うためのクラス
 *
 *   mdarray_ucharクラスは，unsigned char型の多次元配列を IDL や Python のように
 ...
 * 
 * @note  mdarray_ucharクラスは mdarray(基底クラス) を継承しています．<br>
 *        SIMD命令(SSE2)により，基本的な部分が高速化されています．
 * @author  Chisato YAMAUCHI
 * @date  2013-04-01 00:00:00
 */

  class mdarray_uchar : public mdarray
  {

  public:
    /* constructor                                                           */
    /* The mdarray_uchar class has two kinds of operating modes:            */
    ...
    mdarray_uchar();
    mdarray_uchar( bool auto_resize );
    mdarray_uchar( bool auto_resize, unsigned char **extptr_address );
    mdarray_uchar( bool auto_resize, unsigned char *const **extptr2d_address );
    mdarray_uchar( bool auto_resize, unsigned char *const *const **extptr3d_address );
    mdarray_uchar( bool auto_resize, const size_t naxisx[], size_t ndim, bool init_buf );
    mdarray_uchar( bool auto_resize, size_t naxis0 );
    mdarray_uchar( bool auto_resize, size_t naxis0, size_t naxis1 );
    mdarray_uchar( bool auto_resize, size_t naxis0, size_t naxis1, size_t naxis2 );
    mdarray_uchar( bool auto_resize, size_t naxis0, 
...
#if 0
    mdarray_uchar( bool auto_resize, const char *dim_info, int v0, ... );
#endif
    mdarray_uchar( const mdarray_uchar &obj );


    /* --------------------------------------------------------------------- */

    /* `virtual' がついてるものはオーバーライドではなくオーバーロード     */
    /*   mdarray_uchar &operator=(const mdarray_uchar &obj);            */
    /* が隠れて存在する事に注意 (演算子 = だけは，特別な規則があるらしい) */

    /* substitute an array (copy the attribute, too)                      */
    virtual mdarray_uchar &operator=(const mdarray &obj);

    /* (overridden) */
    mdarray_uchar &operator+=(const mdarray &obj);
    mdarray_uchar &operator-=(const mdarray &obj);
    mdarray_uchar &operator*=(const mdarray &obj);
    mdarray_uchar &operator/=(const mdarray &obj);

    /* (overridden) */
    mdarray_uchar &operator=(dcomplex v);
    mdarray_uchar &operator=(double v);
    mdarray_uchar &operator=(long long v);
    mdarray_uchar &operator=(long v);
    mdarray_uchar &operator=(int v);
    mdarray_uchar &operator+=(dcomplex v);
    mdarray_uchar &operator+=(double v);
    mdarray_uchar &operator+=(long long v);
    mdarray_uchar &operator+=(long v);
    ...
    */
    ...
    /* <overloaded>                */
    virtual mdarray_uchar &init( bool auto_resize );
    virtual mdarray_uchar &init( bool auto_resize, const size_t naxisx[], size_t ndim, bool init_buf );
    virtual mdarray_uchar &init( bool auto_resize, size_t naxis0 );
    virtual mdarray_uchar &init( bool auto_resize, size_t naxis0, size_t naxis1 );
    virtual mdarray_uchar &init( bool auto_resize, size_t naxis0, size_t naxis1, size_t naxis2 );
    virtual mdarray_uchar &init( bool auto_resize, size_t naxis0, 
				 const unsigned char vals[] );
    virtual mdarray_uchar &init( bool auto_resize, size_t naxis0, size_t naxis1,
				 const unsigned char vals[] );
    ...
    virtual mdarray_uchar &init( bool auto_resize, size_t naxis0, size_t naxis1, size_t naxis2,
				 const unsigned char *const *const vals[] );
#if 0
    virtual mdarray_uchar &init( bool auto_resize, const char *dim_info,
				 int v0, ... );
    virtual mdarray_uchar &vinit( bool auto_resize, const char *dim_info,
				  int v0, va_list );
#endif
    /*  mdarray_uchar &init( const mdarray &obj ); が隠れている */
    /*  (↑mdarrayクラスのポインタ経由で呼び出し可能)            */

    /* (overridden)                                           */
    mdarray_uchar &init( const mdarray &obj );

    /* copy properties (auto_resize, auto_init, and rounding) */
    /* (overridden)                                           */
    mdarray_uchar &init_properties( const mdarray &src_obj );

    /* setting of the resize mode */
    /* (overridden)               */
    mdarray_uchar &set_auto_resize( bool tf );

    /* setting of the initialize mode */
    /* (overridden)                   */
    mdarray_uchar &set_auto_init( bool tf );

    /* setting of the rounding off possibility */
    /* (overridden)                            */
    mdarray_uchar &set_rounding( bool tf );

    /* setting of strategy of memory allocation */
    /* "auto", "min" and "pow" can be set.      */
    /* (overridden)                             */
    mdarray_uchar &set_alloc_strategy( const char *strategy );

    /* convert the value of the full array element */
    /* <overloaded>                                */
    virtual mdarray_uchar &convert_via_udf(
		void (*func)(const unsigned char *,unsigned char *,size_t,int,void *),
		void *user_ptr );

    /* swap contents between self and another objects */
    /* (overridden)                 */
    mdarray_uchar &swap( mdarray &sobj );
    ...
    /* (overridden)                             */
    mdarray_uchar &cut( mdarray *dest );

    /* interchange rows and columns */
    /* (overridden)                 */
    mdarray_uchar &transpose_xy();

    /* interchange xyz to zxy */
    /* (overridden)           */
    mdarray_uchar &transpose_xyz2zxy();

    /* rotate image                                              */
    /*   angle: 90,-90, or 180                                   */
    /*          (anticlockwise when image is bottom-left origin) */
    /* (overridden)                                              */
    mdarray_uchar &rotate_xy( int angle );
    ...
    */
    ...
    /* (overridden)                                          */
    mdarray_uchar &assign( dcomplex value,
			    ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF,
			    ssize_t idx2 = MDARRAY_INDEF );
    mdarray_uchar &assign( double value, 
			    ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF,
			    ssize_t idx2 = MDARRAY_INDEF );

    /* a reference to the specified value of the element (1 dimension) */
    virtual unsigned char &operator[]( ssize_t idx0 );
    virtual const unsigned char &operator[]( ssize_t idx0 ) const;

    /* a reference to the specified value of the element (1-3 dimensions) */
    virtual unsigned char &operator()( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				ssize_t idx2 = MDARRAY_INDEF);
    virtual const unsigned char &operator()( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				ssize_t idx2 = MDARRAY_INDEF) const;

    /* a reference to the specified value of the element (1-3 dimensions) */
    virtual unsigned char &at( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
			  ssize_t idx2 = MDARRAY_INDEF);
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const unsigned char &at( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				 ssize_t idx2 = MDARRAY_INDEF) const;
#endif
    virtual const unsigned char &at_cs( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				 ssize_t idx2 = MDARRAY_INDEF) const;
    ...
    /* <overloaded>                                */
    virtual mdarray_uchar &put( unsigned char value, ssize_t idx, size_t len );
    virtual mdarray_uchar &put( unsigned char value,
				size_t dim_index, ssize_t idx, size_t len );

    /* copy the array stored in object into the programmer's buffer. */
    /* <overloaded>                                                  */
    virtual ssize_t get_elements( unsigned char *dest_buf, size_t elem_size, 
				ssize_t idx0 = 0, ssize_t idx1 = MDARRAY_INDEF,
				ssize_t idx2 = MDARRAY_INDEF ) const;

    /* copy the array in the programmer's buffer into object's buffer. */
    /* <overloaded>                                                    */
    virtual ssize_t put_elements( const unsigned char *src_buf, size_t elem_size,
				ssize_t idx0 = 0, ssize_t idx1 = MDARRAY_INDEF,
				ssize_t idx2 = MDARRAY_INDEF );
    ...
    /* (overridden)                      */
    mdarray_uchar &increase_dim();

    /* reduction of the dimension number */
    /* (overridden)                      */
    mdarray_uchar &decrease_dim();

    /* change the length of the array */
    /* (overridden)                   */
    mdarray_uchar &resize( size_t len );
    mdarray_uchar &resize( size_t dim_index, size_t len );
    mdarray_uchar &resize( const mdarray &src );

    /* change the length of the 1-d array */
    /* (overridden)                       */
    mdarray_uchar &resize_1d( size_t x_len );

    /* change the length of the 2-d array */
    /* (overridden)                       */
    mdarray_uchar &resize_2d( size_t x_len, size_t y_len );

    /* change the length of the 3-d array */
    /* (overridden)                       */
    mdarray_uchar &resize_3d( size_t x_len, size_t y_len, size_t z_len );

    /* change the length of the array (supports n-dim) */
    /* (overridden)                                    */
    mdarray_uchar &resize( const size_t naxisx[], size_t ndim, 
			    bool init_buf );
    ...
    /* (overridden)                                                   */
    mdarray_uchar &resizef( const char *exp_fmt, ... );
    mdarray_uchar &vresizef( const char *exp_fmt, va_list ap );

    /* change the length of the array relatively */
    /* (overridden)                              */
    mdarray_uchar &resizeby( ssize_t len );			/* for 1-d */
    mdarray_uchar &resizeby( size_t dim_index, ssize_t len );

    /* change the length of the 1-d array relatively */
    /* (overridden)                                  */
    mdarray_uchar &resizeby_1d( ssize_t x_len );

    /* change the length of the 2-d array relatively */
    /* (overridden)                                  */
    mdarray_uchar &resizeby_2d( ssize_t x_len, ssize_t y_len );

    /* change the length of the 3-d array relatively */
    /* (overridden)                                  */
    mdarray_uchar &resizeby_3d(ssize_t x_len, ssize_t y_len, ssize_t z_len);
    ...
    /* (overridden)                                                         */
    mdarray_uchar &resizeby( const ssize_t naxisx_rel[], size_t ndim, 
			       bool init_buf );
    ...
    /* length of first dimension.                                      */
    /* (overridden)                                                    */
    mdarray_uchar &resizebyf( const char *exp_fmt, ... );
    mdarray_uchar &vresizebyf( const char *exp_fmt, va_list ap );

    /* change length of array without adjusting buffer contents */
    /* (overridden)                                             */
    mdarray_uchar &reallocate( const size_t naxisx[], size_t ndim, 
				bool init_buf );

    /* free current buffer and alloc new memory */
    /* (overridden)                             */
    mdarray_uchar &allocate( const size_t naxisx[], size_t ndim, 
			      bool init_buf );

    /* setting of the initial value for .resize(), etc. */
    /* <overloaded>                                     */
    virtual mdarray_uchar &assign_default( unsigned char value );

    /* insert a blank section */
    /* (overridden)           */
    mdarray_uchar &insert( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_uchar &insert( size_t dim_index, ssize_t idx, size_t len );

    /* erase a section */
    /* (overridden)    */
    mdarray_uchar &erase( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_uchar &erase( size_t dim_index, ssize_t idx, size_t len );

    /* copy values between elements (without automatic resizing) */
    /* 移動 (バッファのサイズは変更しない)                       */
    /* (overridden)                                              */
    mdarray_uchar &move( ssize_t idx_src, size_t len, ssize_t idx_dst,
			   bool clr );				/* for 1-d */
    mdarray_uchar &move( size_t dim_index, 
    ...
    /* (overridden)                                           */
    mdarray_uchar &cpy( ssize_t idx_src, size_t len, ssize_t idx_dst,
			  bool clr );				/* for 1-d */
    mdarray_uchar &cpy( size_t dim_index, 
			  ssize_t idx_src, size_t len, ssize_t idx_dst,
			  bool clr );

    /* replace values between elements */
    /* (overridden)                    */
    mdarray_uchar &swap( ssize_t idx_src, size_t len, 
			  ssize_t idx_dst );			/* for 1-d */
    mdarray_uchar &swap( size_t dim_index, 
			   ssize_t idx_src, size_t len, ssize_t idx_dst );

    /* extract a section           */
    /* see also trimf() and trim() */
    /* (overridden)                */
    mdarray_uchar &crop( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_uchar &crop( size_t dim_index, ssize_t idx, size_t len );

    /* flip a section   */
    /* see also flipf() */
    /* (overridden)     */
    mdarray_uchar &flip( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_uchar &flip( size_t dim_index, ssize_t idx, size_t len );
    ...
    */
    ...
    /* Flipping elements is supported */
    /* (overridden)                   */
    mdarray_uchar &trimf( const char *exp_fmt, ... );
    mdarray_uchar &vtrimf( const char *exp_fmt, va_list ap );

    /* flip elements in a section */
    /* (overridden)               */
    mdarray_uchar &flipf( const char *exp_fmt, ... );
    mdarray_uchar &vflipf( const char *exp_fmt, va_list ap );
    ...
    /* (overridden)                        */
    mdarray_uchar &cleanf( const char *exp_fmt, ... );
    mdarray_uchar &vcleanf( const char *exp_fmt, va_list ap );

    /* rewrite element values with a value in a section */
    /* (overridden)                                     */
    mdarray_uchar &fillf( double value, const char *exp_fmt, ... );
    mdarray_uchar &vfillf( double value, const char *exp_fmt, va_list ap );

    /* <overloaded>                                     */
    virtual mdarray_uchar &fillf_via_udf( double value, 
	void (*func)(double [],double,size_t, ssize_t,ssize_t,ssize_t,mdarray_uchar *,void *),
        void *user_ptr, const char *exp_fmt, ... );
    virtual mdarray_uchar &vfillf_via_udf( double value, 
	void (*func)(double [],double,size_t, ssize_t,ssize_t,ssize_t,mdarray_uchar *,void *),
        void *user_ptr, const char *exp_fmt, va_list ap );

    /* add a scalar value to element values in a section */
    /* (overridden)                                      */
    mdarray_uchar &addf( double value, 
			  const char *exp_fmt, ... );
    mdarray_uchar &vaddf( double value, 
			   const char *exp_fmt, va_list ap );

    /* subtract a scalar value from element values in a section */
    /* (overridden)                                             */
    mdarray_uchar &subtractf( double value, 
			       const char *exp_fmt, ... );
    mdarray_uchar &vsubtractf( double value, 
				const char *exp_fmt, va_list ap );

    /* multiply element values in a section by a scalar value */
    /* (overridden)                                           */
    mdarray_uchar &multiplyf( double value, 
			       const char *exp_fmt, ... );
    mdarray_uchar &vmultiplyf( double value, 
				const char *exp_fmt, va_list ap );

    /* divide element values in a section by a scalar value */
    /* (overridden)                                         */
    mdarray_uchar &dividef( double value, 
			     const char *exp_fmt, ... );
    mdarray_uchar &vdividef( double value, 
			      const char *exp_fmt, va_list ap );
    ...
    /* (overridden)                                                          */
    mdarray_uchar &pastef( const mdarray &src,
			    const char *exp_fmt, ... );
    mdarray_uchar &vpastef( const mdarray &src,
			     const char *exp_fmt, va_list ap );
    ...
    /*   8. user pointer                                                     */
    /* <overloaded>                                                          */
    virtual mdarray_uchar &pastef_via_udf( const mdarray &src,
	void (*func)(double [],double [],size_t, ssize_t,ssize_t,ssize_t,mdarray_uchar *,void *),
	void *user_ptr,
        const char *exp_fmt, ... );
    virtual mdarray_uchar &vpastef_via_udf( const mdarray &src,
	void (*func)(double [],double [],size_t, ssize_t,ssize_t,ssize_t,mdarray_uchar *,void *),
	void *user_ptr,
        const char *exp_fmt, va_list ap );
    ...
    /* add an array object */
    /* (overridden)        */
    mdarray_uchar &addf( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_uchar &vaddf( const mdarray &src_img, const char *exp_fmt, va_list ap );

    /* subtract an array object */
    /* (overridden)             */
    mdarray_uchar &subtractf( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_uchar &vsubtractf( const mdarray &src_img, const char *exp_fmt, va_list ap );

    /* multiply an array object */
    /* (overridden)             */
    mdarray_uchar &multiplyf( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_uchar &vmultiplyf( const mdarray &src_img, const char *exp_fmt, va_list ap );

    /* divide an array object */
    /* (overridden)           */
    mdarray_uchar &dividef( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_uchar &vdividef( const mdarray &src_img, const char *exp_fmt, va_list ap );
    ...
    */
    ...
    /* rewrite element values with a value in a section */
    /* (overridden)                                     */
    mdarray_uchar &fill( double value,
			 ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
			 ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
			 ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );

    /* <overloaded>                                     */
    virtual mdarray_uchar &fill_via_udf( double value,
	void (*func)(double [],double,size_t, ssize_t,ssize_t,ssize_t,mdarray_uchar *,void *),
	void *user_ptr,
	ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
	ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
    ...
    /* add a scalar value to element values in a section */
    /* (overridden)                                      */
    mdarray_uchar &add( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* subtract a scalar value from element values in a section */
    /* (overridden)                                             */
    mdarray_uchar &subtract( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* multiply element values in a section by a scalar value */
    /* (overridden)                                           */
    mdarray_uchar &multiply( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* divide element values in a section by a scalar value */
    /* (overridden)                                         */
    mdarray_uchar &divide( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* paste up an array object */
    /* (overridden)             */
    mdarray_uchar &paste( const mdarray &src,
	ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* <overloaded>             */
    virtual mdarray_uchar &paste_via_udf( const mdarray &src,
	void (*func)(double [],double [],size_t,ssize_t,ssize_t,ssize_t,mdarray_uchar *,void *),
	void *user_ptr,
	ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );
    ...
    /* add an array object */
    /* (overridden)        */
    mdarray_uchar &add( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* subtract an array object */
    /* (overridden)             */
    mdarray_uchar &subtract( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* multiply an array object */
    /* (overridden)             */
    mdarray_uchar &multiply( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* divide an array object */
    /* (overridden)           */
    mdarray_uchar &divide( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );
    ...
    */
    ...
    /* <overloaded>                                                    */
    virtual mdarray_uchar &register_extptr( unsigned char **extptr_address );
    virtual mdarray_uchar &register_extptr_2d( unsigned char *const **extptr2d_address );
    virtual mdarray_uchar &register_extptr_3d( unsigned char *const *const **extptr3d_address );

    /* Acquire the specified element's address */
    virtual const unsigned char *carray() const;
    virtual const unsigned char *carray( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF ) const;
    virtual unsigned char *array_ptr();
    virtual unsigned char *array_ptr( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF );
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const unsigned char *array_ptr() const;
    virtual const unsigned char *array_ptr( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF ) const;
#endif
    virtual const unsigned char *array_ptr_cs() const;
    virtual const unsigned char *array_ptr_cs( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF ) const;
    ...
    virtual unsigned char *const *array_ptr_2d( bool use );
    /* followings have no overhead. Execute .array_ptr_2d(true) before use. */
    virtual unsigned char *const *array_ptr_2d();
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const unsigned char *const *array_ptr_2d() const;
#endif
    virtual const unsigned char *const *array_ptr_2d_cs() const;
    ...
    /*        "use=false" when there is no requirement of referring         */
    /*        obtained address table.                                       */
    virtual unsigned char *const *const *array_ptr_3d( bool use );
    /* followings have no overhead. Execute .array_ptr_3d(true) before use. */
    virtual unsigned char *const *const *array_ptr_3d();
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const unsigned char *const *const *array_ptr_3d() const;
#endif
    virtual const unsigned char *const *const *array_ptr_3d_cs() const;
    ...
    */
    ...
    /* (overridden)                    */
    mdarray_uchar &reverse_endian( bool is_little_endian );
    /* (overridden)                    */
    mdarray_uchar &reverse_endian( bool is_little_endian, 
				    size_t begin, size_t length );

    /* acquisition of the initial value */
    virtual unsigned char default_value() const;
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::put( unsigned char value, ssize_t idx, size_t len )
{
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::put( unsigned char value,
				    size_t dim_index, ssize_t idx, size_t len )
{
 ...
 */
inline mdarray_uchar &mdarray_uchar::assign( dcomplex value,
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::assign( double value, 
				     ssize_t idx0, ssize_t idx1, ssize_t idx2 )
{
    this->mdarray::assign(value, idx0, idx1, idx2);
    return *this;
 ...
 *
 */
inline unsigned char &mdarray_uchar::operator[]( ssize_t idx0 )
{
    return ((0 <= idx0 && idx0 < (ssize_t)this->length()) ? 
	    ((unsigned char *)(this->data_ptr()))[idx0] :
	    this->at(idx0));
}
 ...
 *
 */
inline const unsigned char &mdarray_uchar::operator[]( ssize_t idx0 ) const
{
    return ((0 <= idx0 && idx0 < (ssize_t)this->length()) ? 
	    ((const unsigned char *)(this->data_ptr_cs()))[idx0] :
	    this->at_cs(idx0));
}
 ...
 *
 */
inline unsigned char &mdarray_uchar::operator()( ssize_t idx0, ssize_t idx1, ssize_t idx2 )
{
 ...
 *
 */
inline const unsigned char &mdarray_uchar::operator()( ssize_t idx0, ssize_t idx1, ssize_t idx2 ) const
{
 ...
 *
 */
inline unsigned char &mdarray_uchar::at( ssize_t idx0, ssize_t idx1, ssize_t idx2 )
{
    ssize_t idx = this->get_idx_3d(idx0,idx1,idx2);
    if ( 0 <= idx ) {
	return ((unsigned char *)(this->data_ptr()))[idx];
    }
    else {
 ...
 */
#ifdef SLI__OVERLOAD_CONST_AT
inline const unsigned char &mdarray_uchar::at( ssize_t idx0, ssize_t idx1, ssize_t idx2 ) const
{
    return this->at_cs(idx0, idx1, idx2);
}
#endif
 ...
 *
 */
inline const unsigned char &mdarray_uchar::at_cs( ssize_t idx0, ssize_t idx1, ssize_t idx2 ) const
{
    ssize_t idx = this->get_idx_3d_cs(idx0,idx1,idx2);
    if ( 0 <= idx ) {
	return ((const unsigned char *)(this->data_ptr()))[idx];
    }
    else {
	unsigned char *p = (unsigned char *)(this->junk_rec);
	*p = INDEF_UCHAR;
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::ceil()
{
    this->mdarray::ceil();
    return *this;
}
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::floor()
{
    this->mdarray::floor();
    return *this;
}
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::round()
{
    this->mdarray::round();
    return *this;
}
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::trunc()
{
    this->mdarray::trunc();
    return *this;
}
 ...
 *
 */
inline mdarray_uchar &mdarray_uchar::abs()
{
    this->mdarray::abs();
    return *this;
}
 ...
 *
 */
inline const unsigned char *mdarray_uchar::carray() const
{
    return (const unsigned char *)(this->data_ptr_cs());
}
 ...
 *
 */
inline const unsigned char *mdarray_uchar::carray( ssize_t idx0, 
				     ssize_t idx1, ssize_t idx2 ) const
{
    return (const unsigned char *)(this->data_ptr_cs(idx0,idx1,idx2));
}
 ...
 *
 */
inline unsigned char *mdarray_uchar::array_ptr()
{
    return (unsigned char *)(this->data_ptr());
}
 ...
 *
 */
inline unsigned char *mdarray_uchar::array_ptr( ssize_t idx0, 
				  ssize_t idx1, ssize_t idx2 )
{
    return (unsigned char *)(this->data_ptr(idx0,idx1,idx2));
}
 ...
 */
#ifdef SLI__OVERLOAD_CONST_AT
inline const unsigned char *mdarray_uchar::array_ptr() const
{
    return (const unsigned char *)(this->data_ptr_cs());
}
 ...
 *
 */
inline const unsigned char *mdarray_uchar::array_ptr( ssize_t idx0, 
					   ssize_t idx1, ssize_t idx2 ) const
{
    return (const unsigned char *)(this->data_ptr_cs(idx0,idx1,idx2));
}
 ...
 *
 */
inline const unsigned char *mdarray_uchar::array_ptr_cs() const
{
    return (const unsigned char *)(this->data_ptr_cs());
}
 ...
 *
 */
inline const unsigned char *mdarray_uchar::array_ptr_cs( ssize_t idx0, 
					   ssize_t idx1, ssize_t idx2 ) const
{
    return (const unsigned char *)(this->data_ptr_cs(idx0,idx1,idx2));
}
 ...
 *             使用しない場合はfalse(機能off)．
 */
inline unsigned char *const *mdarray_uchar::array_ptr_2d( bool use )
{
    return (unsigned char *const *)(this->mdarray::data_ptr_2d(use));
}

/**
 * @brief  内部で生成された2次元配列用ポインタ配列のアドレスを取得
 */
inline unsigned char *const *mdarray_uchar::array_ptr_2d()
{
    return (unsigned char *const *)(this->mdarray::data_ptr_2d());
}
 ...
 */
#ifdef SLI__OVERLOAD_CONST_AT
inline const unsigned char *const *mdarray_uchar::array_ptr_2d() const
{
    return (const unsigned char *const *)(this->mdarray::data_ptr_2d_cs());
}
#endif
 ...
 * @brief  内部で生成された2次元配列用ポインタ配列のアドレスを取得 (読取専用)
 */
inline const unsigned char *const *mdarray_uchar::array_ptr_2d_cs() const
{
    return (const unsigned char *const *)(this->mdarray::data_ptr_2d_cs());
}
 ...
 *             使用しない場合はfalse(機能off)．
 */
inline unsigned char *const *const *mdarray_uchar::array_ptr_3d( bool use )
{
    return (unsigned char *const *const *)(this->mdarray::data_ptr_3d(use));
}

/**
 * @brief  内部で生成された3次元配列用ポインタ配列のアドレスを取得
 */
inline unsigned char *const *const *mdarray_uchar::array_ptr_3d()
{
    return (unsigned char *const *const *)(this->mdarray::data_ptr_3d());
}
 ...
 */
inline const unsigned char *const *const *mdarray_uchar::array_ptr_3d() const
{
    return (const unsigned char *const *const *)(this->mdarray::data_ptr_3d_cs());
}
#endif
 ...
 * @brief  内部で生成された3次元配列用ポインタ配列のアドレスを取得 (読取専用)
 */
inline const unsigned char *const *const *mdarray_uchar::array_ptr_3d_cs() const
{
    return (const unsigned char *const *const *)(this->mdarray::data_ptr_3d_cs());
}
 ...
 *
 */
inline unsigned char mdarray_uchar::default_value() const
{
    const unsigned char *p = (const unsigned char *)(this->default_value_ptr());
    if ( p == NULL ) return 0;
    else return *p;
}
