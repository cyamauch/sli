/**
  example code
  mdarrayクラス: double *scan_along_x(...), 他
                 任意領域のスキャン
*/
...
#if 0
ssize_t func_1d(double buf[], size_t n, ssize_t x, ssize_t y, ssize_t z,
		const mdarray *obj, void *usr_ptr)
{
    stdstreamio sio;
    size_t i;
    sio.printf("%s: (%zd,%zd,%zd): ",(const char *)usr_ptr,x,y,z);
    for ( i=0 ; i < n ; i++ ) {
	sio.printf("[%g]",buf[i]);
    }
    sio.printf("\n");
...
ssize_t func_2d(double buf[], size_t n0, size_t n1, 
		ssize_t x, ssize_t y ,ssize_t z, const mdarray *obj,
		void *usr_ptr)
{
    stdstreamio sio;
    size_t i,j,ix;
...
ssize_t func_3d(double buf[], size_t n0, size_t n1, size_t n2,
		ssize_t x, ssize_t y ,ssize_t z, const mdarray *obj,
		void *usr_ptr)
{
    stdstreamio sio;
    size_t i,j,k,ix;
    sio.printf("%s: (%zd,%zd,%zd): \n",(const char *)usr_ptr,x,y,z);
    ix = 0;
    for ( i=0 ; i < n2 ; i++ ) {
	for ( j=0 ; j < n1 ; j++ ) {
	    for ( k=0 ; k < n0 ; k++ ) {
...
int main( int argc, char *argv[] )
{
    ...
    sio.printf("*** BEGIN: 63_scan_cols_sample ***\n");
	...
	while ( (buf=myarr.scan_along_x(&n,&x,&y,&z)) != NULL ) {
	...
	while ( (buf=myarr.scan_along_y(&n,&x,&y,&z)) != NULL ) {
	...
	while ( (buf=myarr.scan_along_z(&n,&x,&y,&z)) != NULL ) {
...
*/
	...
	while ( (buf=myarr.scan_zx_planes(&n0,&n1,&x,&y,&z)) != NULL ) {
	...
	while ( (buf=myarr.scan_a_cube(&n0,&n1,&n2,&x,&y,&z)) != NULL ) {
	    sio.printf("%s: (%zd,%zd,%zd): \n","scan_a_cube",x,y,z);
	    ix = 0;
	    for ( i=0 ; i < n2 ; i++ ) {
		for ( j=0 ; j < n1 ; j++ ) {
    ...
    sio.printf("*** END: 63_scan_cols_sample ***\n");
    sio.printf("\n");
