...
#ifndef _SLI__MDARRAY_SIZE_H
#define _SLI__MDARRAY_SIZE_H 1

/**
 * @file   mdarray_size.h
 * @brief  size_t型の多次元配列を扱う mdarray_size クラスのヘッダファイル
 */
...
/*
 * sli::mdarray_size is an inherited class of sli::mdarray, and is a 
...
/**
 * @class  sli::mdarray_size
 * @brief  size_t型の多次元配列を扱うためのクラス
 *
 *   mdarray_sizeクラスは，size_t型の多次元配列を IDL や Python のように
 ...
 * 
 * @note  mdarray_sizeクラスは mdarray(基底クラス) を継承しています．<br>
 *        SIMD命令(SSE2)により，基本的な部分が高速化されています．
 * @author  Chisato YAMAUCHI
 * @date  2013-04-01 00:00:00
 */

  class mdarray_size : public mdarray
  {

  public:
    /* constructor                                                           */
    /* The mdarray_size class has two kinds of operating modes:            */
    ...
    mdarray_size();
    mdarray_size( bool auto_resize );
    mdarray_size( bool auto_resize, size_t **extptr_address );
    mdarray_size( bool auto_resize, size_t *const **extptr2d_address );
    mdarray_size( bool auto_resize, size_t *const *const **extptr3d_address );
    mdarray_size( bool auto_resize, const size_t naxisx[], size_t ndim, bool init_buf );
    mdarray_size( bool auto_resize, size_t naxis0 );
    mdarray_size( bool auto_resize, size_t naxis0, size_t naxis1 );
    mdarray_size( bool auto_resize, size_t naxis0, size_t naxis1, size_t naxis2 );
    mdarray_size( bool auto_resize, size_t naxis0, 
		   const size_t vals[] );
    mdarray_size( bool auto_resize, size_t naxis0, size_t naxis1,
		   const size_t vals[] );
...
#if 0
    mdarray_size( bool auto_resize, const char *dim_info, size_t v0, ... );
#endif
    mdarray_size( const mdarray_size &obj );


    /* --------------------------------------------------------------------- */

    /* `virtual' がついてるものはオーバーライドではなくオーバーロード     */
    /*   mdarray_size &operator=(const mdarray_size &obj);            */
    /* が隠れて存在する事に注意 (演算子 = だけは，特別な規則があるらしい) */

    /* substitute an array (copy the attribute, too)                      */
    virtual mdarray_size &operator=(const mdarray &obj);

    /* (overridden) */
    mdarray_size &operator+=(const mdarray &obj);
    mdarray_size &operator-=(const mdarray &obj);
    mdarray_size &operator*=(const mdarray &obj);
    mdarray_size &operator/=(const mdarray &obj);

    /* (overridden) */
    mdarray_size &operator=(dcomplex v);
    mdarray_size &operator=(double v);
    mdarray_size &operator=(long long v);
    mdarray_size &operator=(long v);
    mdarray_size &operator=(int v);
    mdarray_size &operator+=(dcomplex v);
    mdarray_size &operator+=(double v);
    mdarray_size &operator+=(long long v);
    mdarray_size &operator+=(long v);
    ...
    */
    ...
    /* <overloaded>                */
    virtual mdarray_size &init( bool auto_resize );
    virtual mdarray_size &init( bool auto_resize, const size_t naxisx[], size_t ndim, bool init_buf );
    virtual mdarray_size &init( bool auto_resize, size_t naxis0 );
    virtual mdarray_size &init( bool auto_resize, size_t naxis0, size_t naxis1 );
    virtual mdarray_size &init( bool auto_resize, size_t naxis0, size_t naxis1, size_t naxis2 );
    virtual mdarray_size &init( bool auto_resize, size_t naxis0, 
				 const size_t vals[] );
    virtual mdarray_size &init( bool auto_resize, size_t naxis0, size_t naxis1,
				 const size_t vals[] );
    ...
    virtual mdarray_size &init( bool auto_resize, size_t naxis0, size_t naxis1, size_t naxis2,
				 const size_t *const *const vals[] );
#if 0
    virtual mdarray_size &init( bool auto_resize, const char *dim_info,
				 size_t v0, ... );
    virtual mdarray_size &vinit( bool auto_resize, const char *dim_info,
				  size_t v0, va_list );
#endif
    /*  mdarray_size &init( const mdarray &obj ); が隠れている */
    /*  (↑mdarrayクラスのポインタ経由で呼び出し可能)            */

    /* (overridden)                                           */
    mdarray_size &init( const mdarray &obj );

    /* copy properties (auto_resize, auto_init, and rounding) */
    /* (overridden)                                           */
    mdarray_size &init_properties( const mdarray &src_obj );

    /* setting of the resize mode */
    /* (overridden)               */
    mdarray_size &set_auto_resize( bool tf );

    /* setting of the initialize mode */
    /* (overridden)                   */
    mdarray_size &set_auto_init( bool tf );

    /* setting of the rounding off possibility */
    /* (overridden)                            */
    mdarray_size &set_rounding( bool tf );

    /* setting of strategy of memory allocation */
    /* "auto", "min" and "pow" can be set.      */
    /* (overridden)                             */
    mdarray_size &set_alloc_strategy( const char *strategy );

    /* convert the value of the full array element */
    /* <overloaded>                                */
    virtual mdarray_size &convert_via_udf(
		void (*func)(const size_t *,size_t *,size_t,int,void *),
		void *user_ptr );

    /* swap contents between self and another objects */
    /* (overridden)                 */
    mdarray_size &swap( mdarray &sobj );
    ...
    /* (overridden)                             */
    mdarray_size &cut( mdarray *dest );

    /* interchange rows and columns */
    /* (overridden)                 */
    mdarray_size &transpose_xy();

    /* interchange xyz to zxy */
    /* (overridden)           */
    mdarray_size &transpose_xyz2zxy();

    /* rotate image                                              */
    /*   angle: 90,-90, or 180                                   */
    /*          (anticlockwise when image is bottom-left origin) */
    /* (overridden)                                              */
    mdarray_size &rotate_xy( int angle );
    ...
    */
    ...
    /* (overridden)                                          */
    mdarray_size &assign( dcomplex value,
			    ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF,
			    ssize_t idx2 = MDARRAY_INDEF );
    mdarray_size &assign( double value, 
			    ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF,
			    ssize_t idx2 = MDARRAY_INDEF );

    /* a reference to the specified value of the element (1 dimension) */
    virtual size_t &operator[]( ssize_t idx0 );
    virtual const size_t &operator[]( ssize_t idx0 ) const;

    /* a reference to the specified value of the element (1-3 dimensions) */
    virtual size_t &operator()( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				ssize_t idx2 = MDARRAY_INDEF);
    virtual const size_t &operator()( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				ssize_t idx2 = MDARRAY_INDEF) const;

    /* a reference to the specified value of the element (1-3 dimensions) */
    virtual size_t &at( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
			  ssize_t idx2 = MDARRAY_INDEF);
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const size_t &at( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				 ssize_t idx2 = MDARRAY_INDEF) const;
#endif
    virtual const size_t &at_cs( ssize_t idx0, ssize_t idx1 = MDARRAY_INDEF, 
				 ssize_t idx2 = MDARRAY_INDEF) const;
    ...
    /* <overloaded>                                */
    virtual mdarray_size &put( size_t value, ssize_t idx, size_t len );
    virtual mdarray_size &put( size_t value,
				size_t dim_index, ssize_t idx, size_t len );

    /* copy the array stored in object into the programmer's buffer. */
    /* <overloaded>                                                  */
    virtual ssize_t get_elements( size_t *dest_buf, size_t elem_size, 
				ssize_t idx0 = 0, ssize_t idx1 = MDARRAY_INDEF,
				ssize_t idx2 = MDARRAY_INDEF ) const;

    /* copy the array in the programmer's buffer into object's buffer. */
    /* <overloaded>                                                    */
    virtual ssize_t put_elements( const size_t *src_buf, size_t elem_size,
				ssize_t idx0 = 0, ssize_t idx1 = MDARRAY_INDEF,
				ssize_t idx2 = MDARRAY_INDEF );
    ...
    /* (overridden)                      */
    mdarray_size &increase_dim();

    /* reduction of the dimension number */
    /* (overridden)                      */
    mdarray_size &decrease_dim();

    /* change the length of the array */
    /* (overridden)                   */
    mdarray_size &resize( size_t len );
    mdarray_size &resize( size_t dim_index, size_t len );
    mdarray_size &resize( const mdarray &src );

    /* change the length of the 1-d array */
    /* (overridden)                       */
    mdarray_size &resize_1d( size_t x_len );

    /* change the length of the 2-d array */
    /* (overridden)                       */
    mdarray_size &resize_2d( size_t x_len, size_t y_len );

    /* change the length of the 3-d array */
    /* (overridden)                       */
    mdarray_size &resize_3d( size_t x_len, size_t y_len, size_t z_len );

    /* change the length of the array (supports n-dim) */
    /* (overridden)                                    */
    mdarray_size &resize( const size_t naxisx[], size_t ndim, 
			    bool init_buf );
    ...
    /* (overridden)                                                   */
    mdarray_size &resizef( const char *exp_fmt, ... );
    mdarray_size &vresizef( const char *exp_fmt, va_list ap );

    /* change the length of the array relatively */
    /* (overridden)                              */
    mdarray_size &resizeby( ssize_t len );			/* for 1-d */
    mdarray_size &resizeby( size_t dim_index, ssize_t len );

    /* change the length of the 1-d array relatively */
    /* (overridden)                                  */
    mdarray_size &resizeby_1d( ssize_t x_len );

    /* change the length of the 2-d array relatively */
    /* (overridden)                                  */
    mdarray_size &resizeby_2d( ssize_t x_len, ssize_t y_len );

    /* change the length of the 3-d array relatively */
    /* (overridden)                                  */
    mdarray_size &resizeby_3d(ssize_t x_len, ssize_t y_len, ssize_t z_len);
    ...
    /* (overridden)                                                         */
    mdarray_size &resizeby( const ssize_t naxisx_rel[], size_t ndim, 
			       bool init_buf );
    ...
    /* length of first dimension.                                      */
    /* (overridden)                                                    */
    mdarray_size &resizebyf( const char *exp_fmt, ... );
    mdarray_size &vresizebyf( const char *exp_fmt, va_list ap );

    /* change length of array without adjusting buffer contents */
    /* (overridden)                                             */
    mdarray_size &reallocate( const size_t naxisx[], size_t ndim, 
				bool init_buf );

    /* free current buffer and alloc new memory */
    /* (overridden)                             */
    mdarray_size &allocate( const size_t naxisx[], size_t ndim, 
			      bool init_buf );

    /* setting of the initial value for .resize(), etc. */
    /* <overloaded>                                     */
    virtual mdarray_size &assign_default( size_t value );

    /* insert a blank section */
    /* (overridden)           */
    mdarray_size &insert( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_size &insert( size_t dim_index, ssize_t idx, size_t len );

    /* erase a section */
    /* (overridden)    */
    mdarray_size &erase( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_size &erase( size_t dim_index, ssize_t idx, size_t len );

    /* copy values between elements (without automatic resizing) */
    /* 移動 (バッファのサイズは変更しない)                       */
    /* (overridden)                                              */
    mdarray_size &move( ssize_t idx_src, size_t len, ssize_t idx_dst,
			   bool clr );				/* for 1-d */
    mdarray_size &move( size_t dim_index, 
    ...
    /* (overridden)                                           */
    mdarray_size &cpy( ssize_t idx_src, size_t len, ssize_t idx_dst,
			  bool clr );				/* for 1-d */
    mdarray_size &cpy( size_t dim_index, 
			  ssize_t idx_src, size_t len, ssize_t idx_dst,
			  bool clr );

    /* replace values between elements */
    /* (overridden)                    */
    mdarray_size &swap( ssize_t idx_src, size_t len, 
			  ssize_t idx_dst );			/* for 1-d */
    mdarray_size &swap( size_t dim_index, 
			   ssize_t idx_src, size_t len, ssize_t idx_dst );

    /* extract a section           */
    /* see also trimf() and trim() */
    /* (overridden)                */
    mdarray_size &crop( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_size &crop( size_t dim_index, ssize_t idx, size_t len );

    /* flip a section   */
    /* see also flipf() */
    /* (overridden)     */
    mdarray_size &flip( ssize_t idx, size_t len );		/* for 1-d */
    mdarray_size &flip( size_t dim_index, ssize_t idx, size_t len );
    ...
    */
    ...
    /* Flipping elements is supported */
    /* (overridden)                   */
    mdarray_size &trimf( const char *exp_fmt, ... );
    mdarray_size &vtrimf( const char *exp_fmt, va_list ap );

    /* flip elements in a section */
    /* (overridden)               */
    mdarray_size &flipf( const char *exp_fmt, ... );
    mdarray_size &vflipf( const char *exp_fmt, va_list ap );
    ...
    /* (overridden)                        */
    mdarray_size &cleanf( const char *exp_fmt, ... );
    mdarray_size &vcleanf( const char *exp_fmt, va_list ap );

    /* rewrite element values with a value in a section */
    /* (overridden)                                     */
    mdarray_size &fillf( double value, const char *exp_fmt, ... );
    mdarray_size &vfillf( double value, const char *exp_fmt, va_list ap );

    /* <overloaded>                                     */
    virtual mdarray_size &fillf_via_udf( double value, 
	void (*func)(double [],double,size_t, ssize_t,ssize_t,ssize_t,mdarray_size *,void *),
        void *user_ptr, const char *exp_fmt, ... );
    virtual mdarray_size &vfillf_via_udf( double value, 
	void (*func)(double [],double,size_t, ssize_t,ssize_t,ssize_t,mdarray_size *,void *),
        void *user_ptr, const char *exp_fmt, va_list ap );

    /* add a scalar value to element values in a section */
    /* (overridden)                                      */
    mdarray_size &addf( double value, 
			  const char *exp_fmt, ... );
    mdarray_size &vaddf( double value, 
			   const char *exp_fmt, va_list ap );

    /* subtract a scalar value from element values in a section */
    /* (overridden)                                             */
    mdarray_size &subtractf( double value, 
			       const char *exp_fmt, ... );
    mdarray_size &vsubtractf( double value, 
				const char *exp_fmt, va_list ap );

    /* multiply element values in a section by a scalar value */
    /* (overridden)                                           */
    mdarray_size &multiplyf( double value, 
			       const char *exp_fmt, ... );
    mdarray_size &vmultiplyf( double value, 
				const char *exp_fmt, va_list ap );

    /* divide element values in a section by a scalar value */
    /* (overridden)                                         */
    mdarray_size &dividef( double value, 
			     const char *exp_fmt, ... );
    mdarray_size &vdividef( double value, 
			      const char *exp_fmt, va_list ap );
    ...
    /* (overridden)                                                          */
    mdarray_size &pastef( const mdarray &src,
			    const char *exp_fmt, ... );
    mdarray_size &vpastef( const mdarray &src,
			     const char *exp_fmt, va_list ap );
    ...
    /*   8. user pointer                                                     */
    /* <overloaded>                                                          */
    virtual mdarray_size &pastef_via_udf( const mdarray &src,
	void (*func)(double [],double [],size_t, ssize_t,ssize_t,ssize_t,mdarray_size *,void *),
	void *user_ptr,
        const char *exp_fmt, ... );
    virtual mdarray_size &vpastef_via_udf( const mdarray &src,
	void (*func)(double [],double [],size_t, ssize_t,ssize_t,ssize_t,mdarray_size *,void *),
	void *user_ptr,
        const char *exp_fmt, va_list ap );
    ...
    /* add an array object */
    /* (overridden)        */
    mdarray_size &addf( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_size &vaddf( const mdarray &src_img, const char *exp_fmt, va_list ap );

    /* subtract an array object */
    /* (overridden)             */
    mdarray_size &subtractf( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_size &vsubtractf( const mdarray &src_img, const char *exp_fmt, va_list ap );

    /* multiply an array object */
    /* (overridden)             */
    mdarray_size &multiplyf( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_size &vmultiplyf( const mdarray &src_img, const char *exp_fmt, va_list ap );

    /* divide an array object */
    /* (overridden)           */
    mdarray_size &dividef( const mdarray &src_img, const char *exp_fmt, ... );
    mdarray_size &vdividef( const mdarray &src_img, const char *exp_fmt, va_list ap );
    ...
    */
    ...
    /* rewrite element values with a value in a section */
    /* (overridden)                                     */
    mdarray_size &fill( double value,
			 ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
			 ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
			 ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );

    /* <overloaded>                                     */
    virtual mdarray_size &fill_via_udf( double value,
	void (*func)(double [],double,size_t, ssize_t,ssize_t,ssize_t,mdarray_size *,void *),
	void *user_ptr,
	ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
	ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
    ...
    /* subtract a scalar value from element values in a section */
    /* (overridden)                                             */
    mdarray_size &subtract( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* multiply element values in a section by a scalar value */
    /* (overridden)                                           */
    mdarray_size &multiply( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* divide element values in a section by a scalar value */
    /* (overridden)                                         */
    mdarray_size &divide( double value, 
		    ssize_t col_index = 0, size_t col_size = MDARRAY_ALL,
		    ssize_t row_index = 0, size_t row_size = MDARRAY_ALL,
		    ssize_t layer_index = 0, size_t layer_size = MDARRAY_ALL );
    ...
    /* paste up an array object */
    /* (overridden)             */
    mdarray_size &paste( const mdarray &src,
	ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* <overloaded>             */
    virtual mdarray_size &paste_via_udf( const mdarray &src,
	void (*func)(double [],double [],size_t,ssize_t,ssize_t,ssize_t,mdarray_size *,void *),
	void *user_ptr,
	ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );
    ...
    /* add an array object */
    /* (overridden)        */
    mdarray_size &add( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* subtract an array object */
    /* (overridden)             */
    mdarray_size &subtract( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* multiply an array object */
    /* (overridden)             */
    mdarray_size &multiply( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );

    /* divide an array object */
    /* (overridden)           */
    mdarray_size &divide( const mdarray &src_img,
	  ssize_t dest_col = 0, ssize_t dest_row = 0, ssize_t dest_layer = 0 );
    ...
    */
    ...
    /* <overloaded>                                                    */
    virtual mdarray_size &register_extptr( size_t **extptr_address );
    virtual mdarray_size &register_extptr_2d( size_t *const **extptr2d_address );
    virtual mdarray_size &register_extptr_3d( size_t *const *const **extptr3d_address );

    /* Acquire the specified element's address */
    virtual const size_t *carray() const;
    virtual const size_t *carray( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF ) const;
    virtual size_t *array_ptr();
    virtual size_t *array_ptr( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF );
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const size_t *array_ptr() const;
    virtual const size_t *array_ptr( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF ) const;
#endif
    virtual const size_t *array_ptr_cs() const;
    virtual const size_t *array_ptr_cs( ssize_t idx0, 
	    ssize_t idx1 = MDARRAY_INDEF, ssize_t idx2 = MDARRAY_INDEF ) const;
    ...
    virtual size_t *const *array_ptr_2d( bool use );
    /* followings have no overhead. Execute .array_ptr_2d(true) before use. */
    virtual size_t *const *array_ptr_2d();
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const size_t *const *array_ptr_2d() const;
#endif
    virtual const size_t *const *array_ptr_2d_cs() const;
    ...
    /*        "use=false" when there is no requirement of referring         */
    /*        obtained address table.                                       */
    virtual size_t *const *const *array_ptr_3d( bool use );
    /* followings have no overhead. Execute .array_ptr_3d(true) before use. */
    virtual size_t *const *const *array_ptr_3d();
#ifdef SLI__OVERLOAD_CONST_AT
    virtual const size_t *const *const *array_ptr_3d() const;
#endif
    virtual const size_t *const *const *array_ptr_3d_cs() const;
    ...
    */
    ...
    /* (overridden)                    */
    mdarray_size &reverse_endian( bool is_little_endian );
    /* (overridden)                    */
    mdarray_size &reverse_endian( bool is_little_endian, 
 ...
 *
 */
inline mdarray_size &mdarray_size::put( size_t value, ssize_t idx, size_t len )
{
 ...
 *
 */
inline mdarray_size &mdarray_size::put( size_t value,
				    size_t dim_index, ssize_t idx, size_t len )
{
 ...
 */
inline mdarray_size &mdarray_size::assign( dcomplex value,
 ...
 *
 */
inline mdarray_size &mdarray_size::assign( double value, 
				     ssize_t idx0, ssize_t idx1, ssize_t idx2 )
{
    this->mdarray::assign(value, idx0, idx1, idx2);
    return *this;
 ...
 *
 */
inline size_t &mdarray_size::operator[]( ssize_t idx0 )
{
    return ((0 <= idx0 && idx0 < (ssize_t)this->length()) ? 
	    ((size_t *)(this->data_ptr()))[idx0] :
	    this->at(idx0));
}
 ...
 *
 */
inline const size_t &mdarray_size::operator[]( ssize_t idx0 ) const
{
    return ((0 <= idx0 && idx0 < (ssize_t)this->length()) ? 
	    ((const size_t *)(this->data_ptr_cs()))[idx0] :
	    this->at_cs(idx0));
}
 ...
 *
 */
inline size_t &mdarray_size::operator()( ssize_t idx0, ssize_t idx1, ssize_t idx2 )
{
 ...
 *
 */
inline const size_t &mdarray_size::operator()( ssize_t idx0, ssize_t idx1, ssize_t idx2 ) const
{
 ...
 *
 */
inline size_t &mdarray_size::at( ssize_t idx0, ssize_t idx1, ssize_t idx2 )
{
    ssize_t idx = this->get_idx_3d(idx0,idx1,idx2);
    if ( 0 <= idx ) {
	return ((size_t *)(this->data_ptr()))[idx];
    }
    else {
 ...
 */
#ifdef SLI__OVERLOAD_CONST_AT
inline const size_t &mdarray_size::at( ssize_t idx0, ssize_t idx1, ssize_t idx2 ) const
{
    return this->at_cs(idx0, idx1, idx2);
}
#endif
 ...
 *
 */
inline const size_t &mdarray_size::at_cs( ssize_t idx0, ssize_t idx1, ssize_t idx2 ) const
{
    ssize_t idx = this->get_idx_3d_cs(idx0,idx1,idx2);
    if ( 0 <= idx ) {
	return ((const size_t *)(this->data_ptr()))[idx];
    }
    else {
	size_t *p = (size_t *)(this->junk_rec);
	*p = 0;
 ...
 *
 */
inline mdarray_size &mdarray_size::ceil()
{
    this->mdarray::ceil();
    return *this;
}
 ...
 *
 */
inline mdarray_size &mdarray_size::floor()
{
    this->mdarray::floor();
    return *this;
}
 ...
 *
 */
inline mdarray_size &mdarray_size::round()
{
    this->mdarray::round();
    return *this;
}
 ...
 *
 */
inline mdarray_size &mdarray_size::trunc()
{
    this->mdarray::trunc();
    return *this;
}
 ...
 *
 */
inline mdarray_size &mdarray_size::abs()
{
    this->mdarray::abs();
    return *this;
}
 ...
 *
 */
inline const size_t *mdarray_size::carray() const
{
    return (const size_t *)(this->data_ptr_cs());
}
 ...
 *
 */
inline const size_t *mdarray_size::carray( ssize_t idx0, 
				     ssize_t idx1, ssize_t idx2 ) const
{
    return (const size_t *)(this->data_ptr_cs(idx0,idx1,idx2));
}
 ...
 *
 */
inline size_t *mdarray_size::array_ptr()
{
    return (size_t *)(this->data_ptr());
}
 ...
 *
 */
inline size_t *mdarray_size::array_ptr( ssize_t idx0, 
				  ssize_t idx1, ssize_t idx2 )
{
    return (size_t *)(this->data_ptr(idx0,idx1,idx2));
}
 ...
 */
#ifdef SLI__OVERLOAD_CONST_AT
inline const size_t *mdarray_size::array_ptr() const
{
    return (const size_t *)(this->data_ptr_cs());
}
 ...
 *
 */
inline const size_t *mdarray_size::array_ptr( ssize_t idx0, 
					   ssize_t idx1, ssize_t idx2 ) const
{
    return (const size_t *)(this->data_ptr_cs(idx0,idx1,idx2));
}
 ...
 *
 */
inline const size_t *mdarray_size::array_ptr_cs() const
{
    return (const size_t *)(this->data_ptr_cs());
}
 ...
 *
 */
inline const size_t *mdarray_size::array_ptr_cs( ssize_t idx0, 
					   ssize_t idx1, ssize_t idx2 ) const
{
    return (const size_t *)(this->data_ptr_cs(idx0,idx1,idx2));
}
 ...
 *             使用しない場合はfalse(機能off)．
 */
inline size_t *const *mdarray_size::array_ptr_2d( bool use )
{
    return (size_t *const *)(this->mdarray::data_ptr_2d(use));
}

/**
 * @brief  内部で生成された2次元配列用ポインタ配列のアドレスを取得
 */
inline size_t *const *mdarray_size::array_ptr_2d()
{
    return (size_t *const *)(this->mdarray::data_ptr_2d());
}
 ...
 */
#ifdef SLI__OVERLOAD_CONST_AT
inline const size_t *const *mdarray_size::array_ptr_2d() const
{
    return (const size_t *const *)(this->mdarray::data_ptr_2d_cs());
}
#endif
 ...
 * @brief  内部で生成された2次元配列用ポインタ配列のアドレスを取得 (読取専用)
 */
inline const size_t *const *mdarray_size::array_ptr_2d_cs() const
{
    return (const size_t *const *)(this->mdarray::data_ptr_2d_cs());
}
 ...
 *             使用しない場合はfalse(機能off)．
 */
inline size_t *const *const *mdarray_size::array_ptr_3d( bool use )
{
    return (size_t *const *const *)(this->mdarray::data_ptr_3d(use));
}

/**
 * @brief  内部で生成された3次元配列用ポインタ配列のアドレスを取得
 */
inline size_t *const *const *mdarray_size::array_ptr_3d()
{
    return (size_t *const *const *)(this->mdarray::data_ptr_3d());
}
 ...
 */
inline const size_t *const *const *mdarray_size::array_ptr_3d() const
{
    return (const size_t *const *const *)(this->mdarray::data_ptr_3d_cs());
}
#endif
 ...
 * @brief  内部で生成された3次元配列用ポインタ配列のアドレスを取得 (読取専用)
 */
inline const size_t *const *const *mdarray_size::array_ptr_3d_cs() const
{
    return (const size_t *const *const *)(this->mdarray::data_ptr_3d_cs());
}
 ...
 *
 */
inline size_t mdarray_size::default_value() const
{
    const size_t *p = (const size_t *)(this->default_value_ptr());
    if ( p == NULL ) return 0;
    else return *p;
}
